...
import { DAILY_LOG_ADD_URL, DAILY_LOG_GET_ALL_URL, SMP_ADD_URL, SMP_GET_ALL_URL, SMR_ADD_URL, SMR_FORM_GETALL, SMR_GET_ALL_URL } from "../constants/api";
import AsyncStorage from '@react-native-async-storage/async-storage';
import { AuthUserInfo, DailyLogModel, SMPFormModel, SMRForm, UserInfo } from "../models/BaseModel";
...
export  async  function getSmpForm(userId: string){
  ...
  export async function dailyLogsAdd(formData : DailyLogModel) {
    const user = await getUserFromDevice();
    const token = user?.jwtToken;
    debugger;
    const response = await axios.post<DailyLogModel>(DAILY_LOG_ADD_URL, formData, 
        { 
          headers: {
              "Authorization": `Bearer ${token}`
            }
...
export  async  function getDailyLog(userId: string){
    const user = await getUserFromDevice();
    const token = user?.jwtToken;
    let url = `${DAILY_LOG_GET_ALL_URL}/${userId}`;
    debugger;
    const requestOptions = {
        method: 'POST',
        headers: { "Authorization": `Bearer ${token}` }
